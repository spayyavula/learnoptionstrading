...
async function closeTrade(request, context) {
    try {
        const user = (0, auth_1.requireAuth)(request);
        const tradeId = request.params.tradeId;
        const body = await request.json();
        if (!tradeId) {
            return (0, response_1.badRequest)('Trade ID is required');
        }
        if (body.exit_price == null || body.exit_price <= 0) {
            return (0, response_1.badRequest)('exit_price must be a positive number');
        }
        // Verify trade exists and belongs to user
        const existingTrade = await (0, database_1.queryOne)('SELECT id, user_id, trade_type, entry_price, quantity, exit_date FROM trade_history WHERE id = $1', [tradeId]);
        if (!existingTrade) {
            return (0, response_1.notFound)('Trade not found');
        }
        if (existingTrade.user_id !== user.userId) {
            return (0, response_1.forbidden)('You do not have permission to close this trade');
        }
        if (existingTrade.exit_date) {
            return (0, response_1.badRequest)('Trade has already been closed');
        }
        ...
        const trade = await (0, database_1.queryOne)(`UPDATE trade_history
       SET exit_price = $1,
           exit_date = $2,
           profit_loss = $3,
           is_winner = $4,
       ...
       RETURNING *`, [
        ...
        return (0, response_1.jsonResponse)({ trade });
    }
    catch (error) {
...
async function updateTradingMetrics(userId) {
    // Recalculate trading metrics
    await (0, database_1.queryOne)(`INSERT INTO user_trading_metrics (user_id, total_trades, winning_trades, losing_trades, average_win, average_loss, win_rate, win_loss_ratio, kelly_percentage)
     SELECT
       $1 as user_id,
     ...
     ON CONFLICT (user_id) DO UPDATE SET
       ...
       win_loss_ratio = EXCLUDED.win_loss_ratio`, [userId]);
}
functions_1.app.http('closeTrade', {
    methods: ['PUT'],
    authLevel: 'anonymous',
    route: 'trading/trades/{tradeId}/close',
    handler: closeTrade,
});
//# sourceMappingURL=closeTrade.js.map