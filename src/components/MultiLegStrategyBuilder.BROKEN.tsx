...
export default function MultiLegStrategyBuilder({
  ...
  console.log('🔴🔴🔴 MULTI-LEG BUILDER LOADED 🔴🔴🔴', {
    strategyName,
  ...
  if (strategyName === 'Bull Call Spread') {
    console.log('✅✅✅ BULL CALL SPREAD MATCHED! ✅✅✅')
  } else {
    console.log('❌ Strategy name does NOT match Bull Call Spread:', strategyName)
  }
  ...
  const expiryContracts = useMemo(
    () => {
      const filtered = underlyingContracts.filter(
        c => c.expiration_date === selectedExpiry &&
             (c.open_interest > 0 || c.volume > 0) &&
             !isContractExpired(c.expiration_date)
      )
      console.log('🟢 expiryContracts filtered:', {
        selectedUnderlying,
        selectedExpiry,
        underlyingContractsCount: underlyingContracts.length,
        expiryContractsCount: filtered.length,
        sample: filtered.slice(0, 3).map(c => ({ ticker: c.ticker, type: c.contract_type, strike: c.strike_price }))
      })
      return filtered
    },
    [underlyingContracts, selectedExpiry, selectedUnderlying]
  )

  // Pre-calculate ALL data at component level - hooks CANNOT be inside render functions!
  const calls = useMemo(
    () => {
      console.log(`[DEBUG] expiryContracts raw data:`, expiryContracts.slice(0, 5).map(c => ({
        ticker: c.ticker,
        type: c.contract_type,
        strike: c.strike_price
      })))

      const filtered = expiryContracts.filter(c => {
        const isCall = c.contract_type === 'call'
        if (!isCall) {
          console.log(`[DEBUG] Filtering OUT non-call:`, c.ticker, c.contract_type, c.strike_price)
        }
        return isCall
      }).sort((a, b) => a.strike_price - b.strike_price)

      console.log(`[DEBUG] Calls array for ${strategyName}:`, filtered.length, 'calls found')
      console.log(`[DEBUG] First 3 calls:`, filtered.slice(0, 3).map(c => ({
        ticker: c.ticker,
        type: c.contract_type,
        strike: c.strike_price
      })))

      // Check if any PUTs leaked in
      const putsInCalls = filtered.filter(c => c.contract_type !== 'call')
      if (putsInCalls.length > 0) {
        console.error('🚨 CRITICAL BUG: PUTs found in calls array!', putsInCalls)
      }
      return filtered
    },
    [expiryContracts, strategyName]
  )

  const puts = useMemo(
    () => {
      const filtered = expiryContracts.filter(c => c.contract_type === 'put').sort((a, b) => b.strike_price - a.strike_price)
      console.log(`[DEBUG] Puts array for ${strategyName}:`, filtered.length, 'puts found')
      console.log(`[DEBUG] First 3 puts:`, filtered.slice(0, 3).map(c => ({
        ticker: c.ticker,
        type: c.contract_type,
        strike: c.strike_price
      })))
  ...
  const displayCalls = useMemo(() => {
    const sliced = calls.slice(0, (leg1Page + 1) * PAGE_SIZE)
    console.log(`[DEBUG] displayCalls for ${strategyName}:`, sliced.length, 'items')
    // Double-check for PUTs
    const putsInDisplay = sliced.filter(c => c.contract_type !== 'call')
    if (putsInDisplay.length > 0) {
      console.error('🚨 CRITICAL: PUTs in displayCalls!', putsInDisplay.map(p => ({ ticker: p.ticker, type: p.contract_type, strike: p.strike_price })))
    }
    return sliced
  }, [calls, leg1Page, strategyName])
  ...
  const handleUnderlyingChange = (underlying: string) => {
    ...
    setLegs([])
    // Reset pagination when underlying changes
    setLeg1Page(0)
    setLeg2Page(0)
  }
  ...
  const handleExpiryChange = (expiry: string) => {
    ...
    setLegs([])
    // Reset pagination when expiry changes
    setLeg1Page(0)
    setLeg2Page(0)
  }

  // Reset pagination when strategy changes
  useEffect(() => {
    setLeg1Page(0)
    setLeg2Page(0)
    setLegs([])
  ...
  const renderBullCallSpreadBuilder = () => {
    console.log('🚀🚀🚀 NEW DROPDOWN UI RENDERING FOR BULL CALL SPREAD', { callsCount: calls.length, buyLeg, sellLeg })
    ...
    // ⚡ PERFORMANCE FIX: Limit dropdowns to max 20 strikes to prevent browser freeze
    const limitedCalls = calls.slice(0, 20)

    // Available sell strikes (higher than buy leg) - also limited
    const availableSellStrikes = buyLeg
      ? calls.filter(c => c.strike_price > buyLeg.contract.strike_price).slice(0, 20)
      : []
    ...
    return (
      <div className="space-y-6">
        ...
        {/* Clean 2-Column Layout like Sensibull */}
        <div className="bg-white rounded-xl shadow-lg border-2 border-gray-200 overflow-hidden">
          {/* Header */}
          <div className="bg-gradient-to-r from-green-600 to-green-700 px-6 py-4">
            <h3 className="text-2xl font-bold text-white">🚀 NEW Bull Call Spread Builder 🚀</h3>
            <p className="text-green-100 text-sm mt-1 font-bold">✅ Select strikes for both legs on ONE page</p>
          </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      const contract = limitedCalls.find(c => c.ticker === e.target.value)
                  ...
                  >
                    ...
                    {limitedCalls.map(c => (
                      <option key={c.ticker} value={c.ticker}>
                        ${c.strike_price} - Premium: ${c.last.toFixed(2)}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                {buyLeg && (
                  <div className="bg-green-100 border-2 border-green-300 rounded-lg p-4 space-y-2">
                    <div className="flex justify-between">
                      <span className="text-sm font-medium text-gray-700">Strike:</span>
                      <span className="text-sm font-bold text-gray-900">${buyLeg.contract.strike_price}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm font-medium text-gray-700">Premium:</span>
                      <span className="text-sm font-bold text-green-700">${buyLeg.contract.last.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm font-medium text-gray-700">Type:</span>
                      <span className="text-sm font-bold text-green-700">CALL</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm font-medium text-gray-700">Action:</span>
                      <span className="text-sm font-bold text-green-700">BUY</span>
                    </div>
                    <div className="flex justify-between items-center pt-2 border-t border-green-200">
                      <span className="text-sm font-medium text-gray-700">Total Cost:</span>
                      <span className="text-base font-bold text-red-700">-${(buyLeg.contract.last * 100).toFixed(2)}</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      const contract = availableSellStrikes.find(c => c.ticker === e.target.value)
                  ...
                  >
                    <option value="">
                      {buyLeg ? 'Select strike...' : 'Select buy leg first'}
                    </option>
                    {availableSellStrikes.map((c, idx) => (
                      <option key={c.ticker} value={c.ticker}>
                        ${c.strike_price} - Premium: ${c.last.toFixed(2)} {idx === 19 && availableSellStrikes.length >= 20 ? '(showing first 20)' : ''}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                {sellLeg && (
                  <div className="bg-red-100 border-2 border-red-300 rounded-lg p-4 space-y-2">
                    <div className="flex justify-between">
                      <span className="text-sm font-medium text-gray-700">Strike:</span>
                      <span className="text-sm font-bold text-gray-900">${sellLeg.contract.strike_price}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm font-medium text-gray-700">Premium:</span>
                      <span className="text-sm font-bold text-red-700">${sellLeg.contract.last.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm font-medium text-gray-700">Type:</span>
                      <span className="text-sm font-bold text-red-700">CALL</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm font-medium text-gray-700">Action:</span>
                      <span className="text-sm font-bold text-red-700">SELL</span>
                    </div>
                    <div className="flex justify-between items-center pt-2 border-t border-red-200">
                      <span className="text-sm font-medium text-gray-700">Total Credit:</span>
                      <span className="text-base font-bold text-green-700">+${(sellLeg.contract.last * 100).toFixed(2)}</span>
                    </div>
                  </div>
                )}
              </div>
  ...
  const renderStrategyBuilder = () => {
    console.log('🔷🔷🔷 renderStrategyBuilder called 🔷🔷🔷', {
      selectedExpiry,
      strategyName,
      willRender: !!selectedExpiry,
      timestamp: new Date().toISOString()
    })

    if (!selectedExpiry) {
      console.log('⚠️ No expiry selected - returning null')
      return null
    }

    console.log('🔍 About to switch on strategyName:', strategyName, '| Type:', typeof strategyName)

    switch (strategyName) {
      case 'Bull Call Spread':
        console.log('🎯🎯🎯 MATCHED BULL CALL SPREAD CASE - RENDERING NEW UI 🎯🎯🎯')
        return renderBullCallSpreadBuilder()
      case 'Bear Put Spread':
        console.log('🎯 Matched Bear Put Spread case!')
      ...
      default:
        console.log('⚠️ No match, using default case for:', strategyName)
        return (
          ...
          <div className="lg:col-span-1 space-y-6">
            {legs.length > 0 && (
              <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 sticky top-6">
                ...
                <button
                  onClick={() => {
                    if (canProceed && currentValidation) {
                      console.log('Continue button clicked - calling onLegsSelected', { legsCount: legs.length })
                      onLegsSelected(legs, currentValidation)
                    }
                  }}
                  disabled={!canProceed}
