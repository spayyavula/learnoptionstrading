...
import type { IBKRAccountSummary } from './ibkrService'
...
export class IBKRComplianceService {
  ...
  static async checkPatternDayTraderStatus(
    ...
    const dayTradesRemaining1 = parseInt(accountSummary.daytradesremainingt1 || '3')
    const dayTradesRemaining2 = parseInt(accountSummary.daytradesremainingt2 || '3')
    const dayTradesRemaining3 = parseInt(accountSummary.daytradesremainingt3 || '3')
    const dayTradesRemaining4 = parseInt(accountSummary.daytradesremainingt4 || '3')

    const minRemaining = Math.min(dayTradesRemaining1, dayTradesRemaining2, dayTradesRemaining3, dayTradesRemaining4)
    const dayTradeCount = 3 - minRemaining

    const equity = parseFloat(accountSummary.netliquidation || '0')
    const isPatternDayTrader = dayTradeCount >= this.PDT_THRESHOLD || equity >= this.PDT_EQUITY_REQUIREMENT
    const meetsRequirement = equity >= this.PDT_EQUITY_REQUIREMENT

    const status: PatternDayTraderStatus = {
      isPatternDayTrader,
      dayTradeCount,
      dayTradesRemaining: minRemaining,
    ...
    if (isPatternDayTrader && !meetsRequirement) {
      ...
      status.warning = `No day trades remaining. Account will be restricted from day trading until requirements are met.`
    } else if (minRemaining === 1) {
      status.warning = `Only 1 day trade remaining. Use caution to avoid PDT restrictions.`
    }
  ...
  static async checkBuyingPower(
    ...
    const buyingPower = parseFloat(accountSummary.buyingpower || '0')
    const availableFunds = parseFloat(accountSummary.availablefunds || '0')
    const availablePower = Math.max(buyingPower, availableFunds)
  ...
  static async validateOptionsOrder(
    ...
    if (estimatedCost > 0) {
      const buyingPowerCheck = await this.checkBuyingPower(userId, estimatedCost, accountSummary)
      results.push(buyingPowerCheck)
    }

    const pdtStatus = await this.checkPatternDayTraderStatus(userId, accountSummary)
    ...
    const levelCheck = await this.checkTradingLevel(userId, 2, tradingLevel)
  ...
  static async checkDayTradeLimit(
    ...
    const dayTradesRemaining = parseInt(accountSummary.daytradesremainingt1 || '3')
    const equity = parseFloat(accountSummary.netliquidation || '0')
    const dayTradeCount = 3 - dayTradesRemaining

    if (dayTradesRemaining === 0 && equity < this.PDT_EQUITY_REQUIREMENT) {
      return {
        passed: false,
        type: 'day_trade_limit',
        message: `No day trades remaining. Pattern Day Trader account requires minimum $${this.PDT_EQUITY_REQUIREMENT.toLocaleString()} equity. Current equity: $${equity.toLocaleString()}`,
        severity: 'error',
        details: {
          dayTradesRemaining,
          equity,
    ...
    if (dayTradesRemaining === 1) {
      return {
        passed: true,
        type: 'day_trade_warning',
        message: `Warning: Only 1 day trade remaining in the last ${this.TRADING_DAYS_WINDOW} days. Next day trade will trigger Pattern Day Trader rules.`,
        severity: 'warning',
        details: {
          dayTradeCount,
          threshold: this.PDT_THRESHOLD,
          remainingTrades: dayTradesRemaining
        }
      }
    }
    ...
    return {
      ...
      message: `Day trades remaining: ${dayTradesRemaining}`,
      severity: 'info',
      details: {
        dayTradeCount,
        threshold: this.PDT_THRESHOLD,
  ...
  static async validateMarginRequirements(
    ...
    const equity = parseFloat(accountSummary.netliquidation || '0')
    const availableFunds = parseFloat(accountSummary.availablefunds || '0')
    const excessLiquidity = parseFloat(accountSummary.excessliquidity || '0')

    const totalPositionValue = positions.reduce((sum, pos) => {
      return sum + Math.abs(parseFloat(pos.market_value || '0'))
    }, 0)

    const maintMarginReq = parseFloat(accountSummary.maintmarginreq || '0')
    const availableMargin = excessLiquidity

    const passed = availableMargin >= 0
    ...
    return {
      ...
      message: passed
        ...
        : `Insufficient margin. Required: $${maintMarginReq.toFixed(2)}, Available: $${availableMargin.toFixed(2)}`,
      severity: passed ? 'info' : 'error',
      details: {
        equity,
        availableFunds,
  ...
  static async performPreTradeCompliance(
    ...
    const validationResults = await this.validateOptionsOrder(userId, orderDetails, accountSummary, tradingLevel)
    checks.push(...validationResults)

    const dayTradeCheck = await this.checkDayTradeLimit(userId, accountSummary)
