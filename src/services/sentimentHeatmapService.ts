...
export class SentimentHeatmapService {
  ...
  private static async computeHeatmapData(
    ...
    const filteredContracts = contracts.filter(c =>
      liquidTickers.includes(c.underlying_ticker)
    )
    ...
    sentimentScores.forEach(score => {
      scoreMap.set(score.contract_ticker, score)
    })
    ...
    const minSentiment = allScores.length > 0 ? Math.min(...allScores) : -100
    const maxSentiment = allScores.length > 0 ? Math.max(...allScores) : 100
    const avgSentiment = allScores.length > 0
      ? allScores.reduce((a, b) => a + b, 0) / allScores.length
      : 0

    const uniqueTickers = Array.from(new Set(rows.map(r => r.ticker)))
    const uniqueExpiries = Array.from(new Set(rows.map(r => r.expiry_date)))

    return {
      rows,
      tickers: uniqueTickers,
      expiry_dates: uniqueExpiries,
  ...
  private static buildHeatmapRows(
    ...
    contracts.forEach(contract => {
      ...
      if (!groupedByTickerAndExpiry.has(key)) {
        groupedByTickerAndExpiry.set(key, [])
      }
    ...
    groupedByTickerAndExpiry.forEach((contracts, key) => {
      ...
      if (filters.expiry_type && filters.expiry_type !== 'All' && expiryType !== filters.expiry_type) {
        return
      }
  ...
  private static contractToHeatmapCell(
    ...
    if (sentimentData) {
      switch (filters.sentiment_mode) {
        case 'news_only':
          sentimentScore = sentimentData.finbert_sentiment_score
          break
        case 'analyst_only':
          sentimentScore = sentimentData.analyst_sentiment_score
          break
        case 'momentum':
          sentimentScore = sentimentData.sentiment_momentum
          break
        default:
          sentimentScore = sentimentData.composite_sentiment_score
      }
    ...
    if (filters.min_sentiment !== undefined && sentimentScore < filters.min_sentiment) {
      return null
    }
    if (filters.max_sentiment !== undefined && sentimentScore > filters.max_sentiment) {
      return null
    }
    if (filters.min_confidence !== undefined && confidence < filters.min_confidence) {
      return null
    }
    if (filters.strike_range) {
      if (contract.strike_price < filters.strike_range.min || contract.strike_price > filters.strike_range.max) {
        return null
      }
    }
