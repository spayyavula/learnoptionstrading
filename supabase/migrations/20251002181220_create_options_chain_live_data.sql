/*
  ...
  1. `liquid_tickers` - Stores metadata for supported liquid options tickers
     - ticker symbol, name, sector, market cap
     - current_price, last_update timestamp
     - liquidity metrics: avg_daily_volume, avg_open_interest
     - is_active flag for enabling/disabling tickers
  
  2. `options_contracts_live` - Stores live options contract data
     - contract_ticker, underlying_ticker
     - contract_type (call/put), strike_price, expiration_date
     - pricing: bid, ask, last, mark
     - volume, open_interest
     - Greeks: delta, gamma, theta, vega, rho
     - implied_volatility, intrinsic_value, time_value
     - last_trade_timestamp, last_update timestamp
  
  3. `options_expiries` - Stores expiration dates with categorization
     - expiration_date, underlying_ticker
     - expiry_type (0DTE, Daily, Weekly, Monthly, Quarterly, LEAPS)
     - days_to_expiry, business_days_to_expiry
     - is_standard_expiry flag
     - total_volume, total_open_interest across all strikes
