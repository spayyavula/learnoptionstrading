/*
  ...
  1. New Tables
    - `saved_strategies` - User-saved multi-leg options strategies
    - `strategy_templates` - Public and private strategy templates
    - `strategy_shares` - Shareable links for strategies
...
-- Create strategy_templates table (rename to avoid conflict)
CREATE TABLE IF NOT EXISTS saved_strategy_templates (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  name text NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_saved_strategy_templates_public ON saved_strategy_templates(is_public);

-- Enable Row Level Security
ALTER TABLE saved_strategies ENABLE ROW LEVEL SECURITY;
ALTER TABLE saved_strategy_templates ENABLE ROW LEVEL SECURITY;
...
-- Policies for saved_strategy_templates
CREATE POLICY "Anyone can view public templates"
  ON saved_strategy_templates FOR SELECT
  TO authenticated
  USING (is_public = true OR auth.uid() = created_by);

CREATE POLICY "Authenticated users can create templates"
  ON saved_strategy_templates FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = created_by);

CREATE POLICY "Template creators can update own templates"
  ON saved_strategy_templates FOR UPDATE
  TO authenticated
  USING (auth.uid() = created_by)
  WITH CHECK (auth.uid() = created_by);

CREATE POLICY "Template creators can delete own templates"
  ON saved_strategy_templates FOR DELETE
  TO authenticated
  USING (auth.uid() = created_by);
...
CREATE TRIGGER update_saved_strategy_templates_updated_at
  BEFORE UPDATE ON saved_strategy_templates
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();
