/*
  ...
  1. New Tables
    - `saved_strategies`
      - `id` (uuid, primary key) - Unique identifier for each saved strategy
      - `user_id` (uuid, foreign key) - Links to auth.users
      - `strategy_name` (text) - Type of strategy (Bull Call Spread, Iron Condor, etc.)
      - `custom_name` (text, nullable) - User's custom name for this strategy
      - `underlying_ticker` (text) - The underlying asset symbol
      - `expiration_date` (date) - Expiration date for all legs
      - `legs` (jsonb) - Array of strategy legs with contract details
      - `validation_result` (jsonb, nullable) - Cached validation results
      - `notes` (text, nullable) - User's notes about the strategy
      - `is_favorite` (boolean) - Whether user has favorited this strategy
      - `is_template` (boolean) - Whether this is saved as a template
      - `created_at` (timestamptz) - When the strategy was saved
    ...
    - `strategy_templates`
      ...
      - `strategy_type` (text) - Type of strategy
      - `is_public` (boolean) - Whether this template is publicly available
      - `created_by` (uuid, nullable) - User who created the template
      - `legs_config` (jsonb) - Configuration for strategy legs
      - `created_at` (timestamptz)
      - `updated_at` (timestamptz)

    - `strategy_shares`
      - `id` (uuid, primary key) - Unique identifier
      - `strategy_id` (uuid, foreign key) - Links to saved_strategies
      - `share_token` (text, unique) - Unique shareable token
      - `shared_by` (uuid, foreign key) - User who shared the strategy
  ...
  2. Security
    ...
    - Share links are accessible via token without authentication
*/
